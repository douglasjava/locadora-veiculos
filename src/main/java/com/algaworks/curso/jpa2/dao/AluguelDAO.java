...
public class AluguelDAO implements Serializable {
	...
	public List<Aluguel> buscarPorDataDeEntregaEModeloCarro(Date dataEntrega, ModeloCarro modelo) {
		...
		if(dataEntrega != null){
			ParameterExpression<Date> dataEntregaInicial = builder.parameter(Date.class, "dataEntregaInicial"); // select a from Aluguel where a.data = :data -> dataEntregaInicial
